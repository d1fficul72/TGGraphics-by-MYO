...
# Оперативная статистика в памяти (сбрасывается при перезапуске)
_mem: dict = {
    "users": {},      # user_id → {username, name, first_seen, last_seen, actions: []}
    "counts": {},     # feature → int
}
_known_users: set = set()  # id пользователей которым уже отправили уведомление
...
def log_action(user, action: str):
    ...
    # 3. Оперативная статистика в памяти
    uid = user.id
    if uid not in _mem["users"]:
        _mem["users"][uid] = {
    ...
    _mem["users"][uid]["last_seen"] = ts
    _mem["users"][uid]["actions"].append(f"{ts}: {action}")
    if feature:
        _mem["counts"][feature] = _mem["counts"].get(feature, 0) + 1
...
async def cmd_stats(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    total = len(_mem["users"])
    counts = _mem["counts"]

    lines = [f"📊 *Статистика бота*\n"]
    lines.append(f"👥 Всего пользователей: *{total}*\n")

    feat_labels = {
        "эмодзи_мозаика": "◼ Эмодзи-мозаика",
        "стикерпак":      "▪ Стикерпак",
        "кружок":         "◉ Кружок",
        "gif":            "▶ GIF",
        "коллаж":         "◻ Коллаж",
        "текст_эмодзи":   "Ａ Текст→эмодзи",
    }
    ...
    recent = sorted(_mem["users"].values(), key=lambda x: x["last_seen"], reverse=True)[:10]
    for u in recent:
        un = f"@{u['username']}" if u["username"] else u["name"]
        lines.append(f"  · {un} — {u['last_seen']}")

    await update.message.reply_text("\n".join(lines), parse_mode="Markdown")


async def _notify_admin_new_user(bot, user):
    """Уведомляет админа о новом пользователе."""
    if user.id in _known_users:
        return
    _known_users.add(user.id)
    try:
        # Ищем chat_id админа среди известных пользователей
        admin_data = next(
            (v for v in _mem["users"].values()
             if (v.get("username") or "").lower() == ADMIN_USERNAME.lower()),
            None,
        )
        if not admin_data:
            return
        admin_id = next(
            (uid for uid, v in _mem["users"].items()
             if (v.get("username") or "").lower() == ADMIN_USERNAME.lower()),
            None,
        )
        if not admin_id:
            return
        name = f"{user.first_name or ''} {user.last_name or ''}".strip()
        un   = f"@{user.username}" if user.username else "без username"
        await bot.send_message(
            chat_id=admin_id,
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    asyncio.create_task(_notify_admin_new_user(ctx.bot, update.effective_user))
